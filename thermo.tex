...
\begin{tikzpicture}[
  ...
  arrow inside/.style = {postaction=decorate,decoration={markings,mark=at position .55 with \arrow{>}}}
	...
	\coordinate (@5) at (4,3.7);
	\coordinate (@6) at (4,0.8);
	...
	\draw (k) node[above] {K};
	\draw[thick] (0,-0.3) to (1,-0.3) node[right] {Clausius-Rankine Prozess};
	\draw[dashed] (0,-0.7) to (1,-0.7) node[right] {Carnot Prozess};

	\node[dot,label={below right:$1$}] at (@1) {};
	\node[dot,label={below  left:$2$}] at (@2) {};
	\node[dot,label={above  left:$3$}] at (@3) {};
	\node[dot,label={      right:$4$}] at (@4) {};
	\node[dot,label={below right:$5$}] at (@5) {};
	\node[dot,label={below right:$6$}] at (@6) {};

	\draw (0.6,2.3) node[above] {\tiny $p =$ const.} to (0.8,1.583); 
	\draw (3.4,3.4) node[above] {\tiny $p =$ const.} to (3.82,3.2); 
	\draw (2.6,1.5)   node[above] {\small $p,T =$ const.} to (3.2,0.8); 
	\draw (1.6,2.5) to (2,2.1); 

	\draw (-0.2,-1.5) node[right] {$\displaystyle \eta_{th} = 1 - \frac{|q_{61}|}{q_{23} + q_{34} + q_{45}} = 1- \frac{h_6 - h_1}{h_5 -h_2}$};
\end{tikzpicture}
