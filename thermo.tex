...
\begin{tabular}{c|c|l} 
	...
	\multicolumn{3}{c}{$\displaystyle \eta_{th} = 1 - \frac{|q_{ab}|}{q_{zu}} ,\quad \epsilon_K = \frac{q_zu}{w} ,\quad \epsilon_{WP} = \frac{|q_{ab}|}{w} ,\quad \epsilon 	= \frac{v_1}{v_2} ,\quad \psi 		= \frac{p3}{p2} ,\quad \varphi 	= \frac{v_4}{v}$}  \\ \hline
\begin{tikzpicture}[
	scale=0.8, every node/.style={scale=1},
  > = latex,
  dot/.style = {draw,fill,circle,inner sep=1pt},
...
&
	...
	\begin{tikzpicture}
		...
		\draw (1,2) node {$\displaystyle \eta_{th} = 1 - \frac{T_1}{T_2} = 1 - \left(\frac{p_1}{p_2}\right)^{\frac{\kappa-1}{\kappa}}$};
		\draw (1,0);
	\end{tikzpicture}
\\
\end{tabular}
